...
function patrolling(this_sprite) {
  ...
  if (
    getProp(this_sprite, "direction") > 270 ||
    getProp(this_sprite, "direction") < 90
  ) {
    mirrorSprite(this_sprite, "right");
  } else {
    mirrorSprite(this_sprite, "left");
...
showTitleScreen("spring game", "Please press up!");
playSound("sound://category_tap/puzzle_game_organic_metallic_button_1.mp3");

keyPressed("when", "up", function () {
  hideTitleScreen();
  makeBurst(25, "purple bunny", "burst");
  setProp({ costume: "purple bunny" }, "scale", 75);
